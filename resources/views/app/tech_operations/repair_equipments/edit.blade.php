        ...
        <div class="row">
            <div class="col-8">
                <div class="card mw-50">
                    <div class="card-body">
                        <form method="post" action="{{ route($route_parameter . '.repair_equipments.update', ['tech_operation' => $techOperation, 'pivot_id' => $pivot->id]) }}" id="add">
                            @csrf
                            @method('put')
                            <div class="row">
                                <div class="col-12">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control mb-4 @error('repair_equipment_id') is-invalid @enderror" name="repair_equipment_id" required>
                                            @foreach ($repairEquipments as $item)
                                                <option value="{{ $item->id }}" {{ old('repair_equipment_id', $pivot->repair_equipment_id) == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                                            @endforeach
                                        </select>
