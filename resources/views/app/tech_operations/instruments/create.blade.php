        ...
        <div class="row">
            <div class="col-8">
                <div class="card mw-50">
                    <div class="card-body">
                        <form method="post" action="{{ route($route_parameter.'.instruments.store', ['tech_operation' => $techOperation]) }}" id="add">
                            @csrf
                            <div class="row">
                                <div class="col-12">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control mb-4 @error('instrument_id') is-invalid @enderror" name="instrument_id" required>
                                            @foreach ($instruments as $item)
                                                <option value="{{ $item->id }}" {{ old('instrument_id') == $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                                            @endforeach
                                        </select>
                                        @error('instrument_id')
