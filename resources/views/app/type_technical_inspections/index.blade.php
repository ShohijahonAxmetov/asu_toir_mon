    ...
    <div class="container-fluid">
        <div class="card mt-4">
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-hover mb-0">
                        ...
                        @foreach ($type_technical_inspections as $key => $item)
                            <tr>
                                <th scope="row" style="width: 100px">{{ $type_technical_inspections->firstItem() + $key }}</th>
                                <td>
                                    <div class="d-flex align-items-center">
                                        {{ $item->name }}
                                    </div>
                                </td>
                                <td>раз в {{ $item->value }} {{$item->key == 'hours' ? 'часов' : 'месяцов'}}</td>
                ...
                <div class="mt-4">
                    {{ $type_technical_inspections->links() }}
                </div>
            </div>
        </div>
    </div>
