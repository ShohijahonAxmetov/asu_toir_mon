    ...
    <div class="row mb-4">
        <div class="col-lg-12">
            <div class="chart__block card p-4 h-100 h-100">
                ...
                <!-- Modal -->
                <div class="modal fade" id="diagnostikaModal" tabindex="-1" aria-labelledby="diagnostikaModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-xl">
                        <div class="modal-content" id="draggable__diagnostika_modal">
                            <div class="modal-body" id="diagnostikaModalBody">
                                <h3>Планированные</h3>
                                @foreach($zaplanirovannie as $key => $item)
                                <a class="fs-2 text-black mb-3 d-inline-block" name="{{$key}}">{{$item->name}}</a>
                                <table class="table table-striped">
                                    <thead>
                                        <tr>
                                            <th scope="col">#</th>
                                            <th scope="col">НАЗВАНИЕ</th>
                                            <th scope="col">Тип Т.О.</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{$detail->name ?? '--'}}</td>
                                            <td>{{$detail->type_technical_inspection->name ?? '--'}}</td>
                                            <td>{{$detail->planned ?? '--'}}</td>
                                            <td>{{$detail->technical_inspections[0]->now ?? '--'}}</td>
                                            <td>{{$detail->days ? $detail->days : '--'}}</td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                                @endforeach
                                <br>
                                <h3>Фактически проведенные</h3>
                                @foreach($provedennie as $key => $item)
                                <a class="fs-2 text-black mb-3 d-inline-block" name="{{$key}}">{{$item->name}}</a>
                                <table class="table table-striped">
                                    <thead>
                                        <tr>
...
<script>
    let tex_chart = document.getElementById('tex_chart').getContext('2d');
    let tex_chart_bar = new Chart(tex_chart, {
        type: "bar",
        data: {
            labels: ["Первое ТО", "Второе ТО", "Третое ТО", "Сезонное ТО (СО)", "Регл. ремонт - ПР-1", "Регл. ремонт - ПР-2", "Кап. ремонт - КР"],
            datasets: [
                {
                    label: "Запланировано",
                    data: Object.values(JSON.parse('{!!json_encode($result_plan)!!}')),
                    borderColor: '#36A2EB',
                ...
                {
                    label: "Проведено",
                    data: Object.values(JSON.parse('{!!json_encode($result_fact)!!}')),
                    borderColor: '#FF6384',
    ...
    let start_date_tex = document.getElementById('tex_start_date');
    let end_date_tex = document.getElementById('tex_end_date');
    for(let input_tex of [start_date_tex, end_date_tex]) {
        let tex_chart_url = "{{route('chart')}}";
        input_tex.addEventListener('change', (element) => {
            fetch(tex_chart_url + '?start=' + start_date_tex.value + '&end=' + end_date_tex.value)
                .then(response => response.json())
                .then(commits => {
                    tex_chart_bar.data.datasets[0].data = [];
                    tex_chart_bar.data.datasets[0].data = commits.result_plan;
                    tex_chart_bar.data.datasets[1].data = [];
                    tex_chart_bar.data.datasets[1].data = commits.result_fact;
                    tex_chart_bar.update();

                    /*
                        vstavit dannie v modal
                    */
                    let result = '<h3>Планированные</h3>';
                    commits.zaplanirovannie.forEach((item) => {

                        if(item.details && item.details[0]) {
                            result = result + '<a class="fs-2 text-black mb-3 d-inline-block">' + item.name + '</a>';
                            result = result + '<table class="table table-striped">' +
                                                  '<thead>' +
                                                    '<tr>' +
                                                      '<th scope="col">#</th>' +
                            ...
                            item.details.forEach((element) => {
                                result = result + '<tr>' +
                                                      ...
                                                      '<td>' + (element.name ?? '--') + '</td>' +
                                                      '<td>' + (element.type_technical_inspection.name ?? '--') + '</td>' +
                                                      '<td>' + (element.planned ?? '--') + '</td>' +
                                                      '<td>' + ((element.technical_inspections && element.technical_inspections[0]) ? element.technical_inspections[0].now : '--') + '</td>' +
                                                      '<td>' + (element.days ?? '--') + '</td>' +
                                                    '</tr>';
                    ...
                    result = result + '<h3>Фактически проведенные</h3>'
                    commits.provedennie.forEach((item) => {
                     ...
                     $('#diagnostikaModalBody').html('');
                     $('#diagnostikaModalBody').append(result);
                });
        });
    }
</script>
